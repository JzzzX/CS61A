...
def count_coins(total):
    ...
    "*** YOUR CODE HERE ***"
    # 我的答案
    def count_coints_helper(total, coin):
        if total == 0:
            return 1
        elif total < 0 or coin is None:
            return 0
        else:
            return count_coints_helper((total - coin), coin) + count_coints_helper(total, next_smaller_coin(coin))
    
    return count_coints_helper(total, 25)
...
# 官方解答
    def constrained_count(total, smallest_coin):
        ...
        return without_coin + with_coin # 返回两种情况的总和
