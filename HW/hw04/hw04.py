def deep_map(f, s):
    ...
    for i in range(len(s)):
        # 使用 isinstance 内置函数判断 s[i] 是否在列表中
        if isinstance(s[i], list): 
            deep_map(f, s[i]) # 递归调用
        # 如果当前元素是非列表
        else:
            s[i] = f(s[i]) # 直接应用函数 f，并修改原列表中的该元素
...
def balanced(m):
    ...
    "*** YOUR CODE HERE ***"
    if is_planet(m):
         # 星球总是平衡的，因为它们没有挂任何其他东西
        return True
    else:
        # 递归检查移动结构是否平衡
        left_end, right_end = end(left(m)), end(right(m)) # 左右臂的末端
        torque_left = length(left(m)) * total_mass(left_end) # 左臂的扭矩
        torque_right = length(right(m)) * total_mass(right_end)  # 右臂的扭矩

        # 两臂的扭矩必须相等，同时两臂末端的移动结构也必须平衡
