    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if current_ant.can_contain(self):
                # 如果现有蚂蚁是容器，并且它可以容纳当前蚂蚁
                current_ant.store_ant(self)
            elif self.can_contain(current_ant):
                # 如果当前蚂蚁是容器，并且可以容纳现有蚂蚁
                self.store_ant(current_ant)
                place.ant = self  # 更新位置的引用，指向当前的容器蚂蚁
            else:
                # 两只蚂蚁不能同时占据该位置
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place # 从当前 ThrowerAnt 的位置开始检查
        distance = 0 # 记录距离
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        # 遍历蜜蜂并对每只蜜蜂造成反射伤害
        for bee in self.place.bees[:]:  # 使用切片避免修改列表导致的迭代问题
            bee.reduce_health(amount)

        # 如果 FireAnt 生命值在受到这次伤害后会变成 0 或更低，造成额外伤害
        if self.health - amount <= 0:
            for bee in self.place.bees[:]:
                bee.reduce_health(self.damage)

        # 调用父类的 reduce_health 方法 减少 FIreAnt 生命值
...
class HungryAnt(Ant):
    ...
    def __init__(self, health = 1):
        super().__init__(health)
        self.turns_to_chew = 0 # 实例属性，表示当前还有多少回合不能吃

    def action(self, gamestate):
        if self.turns_to_chew > 0:
            # HungryAnt 还在咀嚼，减少咀嚼回合数
            self.turns_to_chew -= 1
        else:
            # HungryAnt 准备吃掉一只蜜蜂
            bee = random_bee(self.place.bees) # 从当前位置选择一只随机的蜜蜂
            if bee: # 如果当前位置有蜜蜂
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        # 每回合对它所在位置的所有蜜蜂造成 1 点伤害
        for bee in self.place.bees[:]:
            bee.reduce_health(self.damage)

        # 如果它包含其他 Ants，则执行该 Ant 的操作
        if self.ant_contained:
            self.ant_contained.action(gamestate)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while place is not None:
            ...
            if ant is not None:
                # 处理容器蚂蚁
                if ant.is_container:
                    if ant.ant_contained and not getattr(ant.ant_contained, 'has_doubled', False):
                        ant.ant_contained.damage *= 2
                        ant.ant_contained.has_doubled = True
                # 处理普通蚂蚁
                if not getattr(ant, 'has_doubled', False):
                    ant.damage *= 2
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        # 调用父类的 reduce_health 方法
        super().reduce_health(amount)
        # 如果蚁后的生命值小于等于 0，则游戏结束
        if self.health <= 0:
            ants_lose()
        # END Problem 12
